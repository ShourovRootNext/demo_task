...
class ConfigurationController extends Controller
{
    ...
    public function storeMetaData($id)
    {
        ...
        if (isset($params['images'])) {
            foreach ($params['images'] as $index => $images) {
                $params['advertisement'][$index] =  $this->uploadAdvertisementImages($images, $links, $index, $advertisement);
            }
        ...
        if(!empty($links)){
            ...
            $params['advertisement'] =  $this->uploadAdvertisementImagesLinks( $links,  $params['advertisement']);
            //dd($params['advertisement'] );
        }
     ...
     * @return array
     */
    public function uploadAdvertisementImages($data, $links, $index, $advertisement)
    {
        $saveImage = [];
    ...
    public function uploadAdvertisementImagesLinks($links, $advertisement)
    {
        $saveImage = [];
        ...
        if($saveData ){
            foreach($saveData as $key=> $list){
              
                foreach($list as $inx => $data){
                    $link = "image_".$inx;
                    $linkTmp = "links_".$inx;
                    $saveData[$key][$linkTmp] = isset($links[$key][$link])?$links[$key][$link]:'';
